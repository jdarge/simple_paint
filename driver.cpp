...
int CANVAS_X = (SCREEN_WIDTH - CANVAS_WIDTH) / 2;
int CANVAS_Y = (SCREEN_HEIGHT - CANVAS_HEIGHT) / 2;
...
void updateCanvasPosition() {
    ...
    CANVAS_WIDTH = SCREEN_WIDTH - 2 * PADDING;
    CANVAS_HEIGHT = SCREEN_HEIGHT - 2 * PADDING;
    CANVAS_X = PADDING;
    CANVAS_Y = PADDING;

    SDL_DestroyTexture(canvasTexture);
    canvasTexture = SDL_CreateTexture(gRenderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, CANVAS_WIDTH, CANVAS_HEIGHT);
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char *args[]) {
    ...
    while (!quit) {
        ...
        if (e.type == SDL_WINDOWEVENT) { // TODO idk, im on i3 rn lmao
